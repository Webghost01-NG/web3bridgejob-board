...
const Field = styled.div`margin-bottom: 1.1rem;`
...
const Register = () => {
  ...
  const [form, setForm] = useState({ displayName: '', email: '', password: '', company: '' })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const res = await dispatch(registerUser({ ...form, role }))
    if (!res.error) navigate(role === 'company' ? '/company/dashboard' : '/candidate/dashboard')
  }
  ...
  return (
    <Page>
      <Card>
        ...
        <Sub>Join the #1 job board for self-taught devs</Sub>
        <RoleToggle>
          <RoleBtn $active={role === 'candidate'} onClick={() => setRole('candidate')} type="button">👨‍💻 I'm a Candidate</RoleBtn>
          <RoleBtn $active={role === 'company'} onClick={() => setRole('company')} type="button">🏢 I'm a Company</RoleBtn>
        </RoleToggle>
        ...
        <form onSubmit={handleSubmit}>
          <Field>
            ...
            <InputWrap>
              <FiUser size={15} />
              <Input name="displayName" placeholder="Your full name" value={form.displayName} onChange={handleChange} required />
            </InputWrap>
          </Field>
          {role === 'company' && (
            <Field>
              ...
              <InputWrap>
                <FiBriefcase size={15} />
                <Input name="company" placeholder="Company Ltd" value={form.company} onChange={handleChange} required />
              </InputWrap>
            </Field>
          )}
          <Field>
